...
def book_service(service_type, service_id):
    ...
    if request.method == 'POST':
        ...
        booking = Booking(user_id=user_id, service_id=service_id, category=service_type, destination=service.name, date=date, guests=guests, status='Pending', paid=False, amount=service.price)
...
def payment(booking_id):
    ...
    if request.method == 'POST':
        ...
        # Simulate Razorpay payment (replace with real integration as needed)
        payment = Payment(user_id=booking.user_id, booking_id=booking.id, service_type=booking.category, service_id=booking.service_id, status='success', amount=booking.amount, method=method, transaction_id='SIMULATED123')
...
def cancel_booking(booking_id):
    ...
    if booking.status == 'Confirmed':
        flash('Cannot cancel a confirmed booking. Please contact support for refund.', 'danger')
    else:
